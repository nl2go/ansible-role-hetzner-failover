...
DOCUMENTATION = """
---
module: nl2go_hcloud_server

short_description: Create and manage cloud servers on the Hetzner Cloud (Workaround - see description)

version_added: "2.8"

description:
    - Create, update and manage cloud servers on the Hetzner Cloud.
    - Copied from the original ansible source
    - Workaround for creating a hcloud server from a custom snapshot for ansible versions earlier than 2.9
    - Custom module is used in tests only, not in the role itself
...
class AnsibleHcloudServer(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "nl2go_hcloud_server")
        self.nl2go_hcloud_server = None

    def _prepare_result(self):
        image = None if self.nl2go_hcloud_server.image is None else to_native(self.nl2go_hcloud_server.image.name)
        return {
            "id": to_native(self.nl2go_hcloud_server.id),
            "name": to_native(self.nl2go_hcloud_server.name),
            "ipv4_address": to_native(self.nl2go_hcloud_server.public_net.ipv4.ip),
            "ipv6": to_native(self.nl2go_hcloud_server.public_net.ipv6.ip),
            "image": image,
            "server_type": to_native(self.nl2go_hcloud_server.server_type.name),
            "datacenter": to_native(self.nl2go_hcloud_server.datacenter.name),
            "location": to_native(self.nl2go_hcloud_server.datacenter.location.name),
            "rescue_enabled": self.nl2go_hcloud_server.rescue_enabled,
            "backup_window": to_native(self.nl2go_hcloud_server.backup_window),
            "labels": self.nl2go_hcloud_server.labels,
            "delete_protection": self.nl2go_hcloud_server.protection["delete"],
            "rebuild_protection": self.nl2go_hcloud_server.protection["rebuild"],
            "status": to_native(self.nl2go_hcloud_server.status),
        }

    def _get_server(self):
        try:
            if self.module.params.get("id") is not None:
                self.nl2go_hcloud_server = self.client.servers.get_by_id(
                    self.module.params.get("id")
                )
            else:
                self.nl2go_hcloud_server = self.client.servers.get_by_name(
    ...
    def _update_server(self):
        try:
            rescue_mode = self.module.params.get("rescue_mode")
            if rescue_mode and self.nl2go_hcloud_server.rescue_enabled is False:
                if not self.module.check_mode:
                    self._set_rescue_mode(rescue_mode)
                self._mark_as_changed()
            elif not rescue_mode and self.nl2go_hcloud_server.rescue_enabled is True:
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.disable_rescue().wait_until_finished()
                self._mark_as_changed()

            if self.module.params.get("backups") and self.nl2go_hcloud_server.backup_window is None:
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.enable_backup().wait_until_finished()
                self._mark_as_changed()
            elif not self.module.params.get("backups") and self.nl2go_hcloud_server.backup_window is not None:
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.disable_backup().wait_until_finished()
                self._mark_as_changed()

            labels = self.module.params.get("labels")
            if labels is not None and labels != self.nl2go_hcloud_server.labels:
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.update(labels=labels)
                self._mark_as_changed()

            server_type = self.module.params.get("server_type")
            if server_type is not None and self.nl2go_hcloud_server.server_type.name != server_type:
                previous_server_status = self.nl2go_hcloud_server.status
                ...
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.change_type(
            ...
            if (delete_protection is not None and rebuild_protection is not None) and (
                    delete_protection != self.nl2go_hcloud_server.protection["delete"] or rebuild_protection !=
                    self.nl2go_hcloud_server.protection["rebuild"]):
                if not self.module.check_mode:
                    self.nl2go_hcloud_server.change_protection(delete=delete_protection,
    ...
    def _set_rescue_mode(self, rescue_mode):
        if self.module.params.get("ssh_keys"):
            resp = self.nl2go_hcloud_server.enable_rescue(type=rescue_mode,
    ...
    def start_server(self):
        try:
            if self.nl2go_hcloud_server.status != Server.STATUS_RUNNING:
                if not self.module.check_mode:
                    self.client.servers.power_on(self.nl2go_hcloud_server).wait_until_finished()
                self._mark_as_changed()
            self._get_server()
        except APIException as e:
            self.module.fail_json(msg=e.message)
    ...
    def stop_server(self):
        try:
            if self.nl2go_hcloud_server.status != Server.STATUS_OFF:
                if not self.module.check_mode:
                    self.client.servers.power_off(self.nl2go_hcloud_server).wait_until_finished()
                self._mark_as_changed()
            self._get_server()
        except APIException as e:
            self.module.fail_json(msg=e.message)
    ...
    def rebuild_server(self):
        ...
        try:
            if not self.module.check_mode:
                self.client.servers.rebuild(self.nl2go_hcloud_server, self.client.images.get_by_name(
    ...
    def present_server(self):
        self._get_server()
        if self.nl2go_hcloud_server is None:
            self._create_server()
        else:
            self._update_server()
    ...
    def delete_server(self):
        try:
            self._get_server()
            if self.nl2go_hcloud_server is not None:
                if not self.module.check_mode:
                    self.client.servers.delete(self.nl2go_hcloud_server).wait_until_finished()
                self._mark_as_changed()
            self.nl2go_hcloud_server = None
        except APIException as e:
            self.module.fail_json(msg=e.message)
